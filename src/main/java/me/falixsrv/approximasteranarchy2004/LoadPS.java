...
public class ProtocolSupport extends JavaPlugin {
	...
	protected static final String supported_platform_version = "1.21.1";
    ...
    Logger logger = getLogger();
		...
		try {
			...
			BIG_ERROR_THAT_ANYONE_CAN_SEE("Unsupported version" + Bukkit.getVersion());
			getLogger().log(Level.SEVERE, "Platform init failed. The Plugin is turning off...", t);
      loaded = false;
			return;
		}
	...
	protected void BIG_ERROR_THAT_ANYONE_CAN_SEE(String message) {
		Logger logger = getLogger();
		logger.severe("╔══════════════════════════════════════════════════════════════════╗");
		logger.severe("║            ViaVersionProtocolSupport: Platform init failed        ");
		logger.severe("║   " + message                                                      );
		logger.severe("║                                                                   ");
		logger.severe("║   This version of plugin only supports                            ");
		logger.severe("║   Minecraft Server version " + supported_platform_version          );
		...
		logger.severe("║                                                                   ");
		logger.severe("║    https://github.com/GAME-CLI-SRV-DEV/ViaVersionProtocolSupport  ");
    logger.severe("║       This Plugin Will Now Shut Down Automaticly...               ");
		logger.severe("╚══════════════════════════════════════════════════════════════════╝");
